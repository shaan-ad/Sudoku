...
$(document).on('mousedown', 'td.sudoku-cell', function(ev) {
  ...
  if (conflicts.length > 0) {
    $('#errors').trigger('log', ['error', {status: "mistake"}]);
    showpopup('#errors');
  }
    ...
    if (countfilled(sofar) == Sudoku.S) {
      showpopup('#victory');
      $('#victory').trigger('log', ['victory']);
    }
    else if (countfilled(sofar) != Sudoku.S) {
    $('#errors').trigger('log', ['noError', {status: "noError"}]);
    }
  }
});
